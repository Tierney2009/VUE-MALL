      ...
      <ul class="orderList" >
        <li v-for="(item,index) in orderList" :key="'order'+item.id">
          ...
          <div class="orderDetail">
            ...
            <button v-if="item.state===0" @click="open">确认付款</button>
...
var paied = require('../../assets/img/pay.jpg')
export default {
  name: 'MyOrder',
  components:{
    Popup
  ...
  methods:{
    open(){
      this.$confirm(
        '<div >'+'订单价格：'+'<span style="color: red">'+this.orderList.item.amount+'</span>'+'</div>' +
        "<img src="+paied+" width=300px height=300px > ",
        '请扫描二维码在线支付'
        ,{
